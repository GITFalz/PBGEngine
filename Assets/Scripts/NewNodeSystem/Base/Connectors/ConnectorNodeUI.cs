    ...
    new BaseNodeUI(node, name, position, color, true, () => [
        new UIVCol(blank_sharp_g_[30], grow_children)[
            new UIVCol(border_[5, 5, 5, 5], grow_children)[
                new UIHCol(grow_children, spacing_[20])[
                    Run(() => {
                        var button = new UIButton(w_[15], h_[15], blank_sharp, rgb_v3_[color], middle_left);
                        var field = new NodeInputField(button, node, new() { Name = "Input", Type = "float", CanOverload = true });
                        button.OnClick(_ => { if (field.Input != null) NodeBase.Connect(field.Input); });
                        node.InputField = field;
                        button.Dataset["field"] = field;
                        return button;
                    }),
                    Run(() => {
                        var button = new UIButton(w_[15], h_[15], blank_sharp, rgb_v3_[color], middle_left);
                        var field = new NodeOutputField(button, node, new() { Name = "Output", Type = "float" });
                        button.OnClick(_ => NodeBase.Connect(field.Output));
                        node.OutputField = field;
                        button.Dataset["field"] = field;
                        return button;
                    })
                ]
