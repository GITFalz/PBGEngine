...
public class PlayerWalkingState(PlayerGameState gameState, PlayerController player) : PlayerGameBase(gameState, player)
{
    private Vector2i _input = Vector2i.Zero;

    public override void Start()
    {
        GameState.PlayerPace = 40f;
        Player.Model?.PlayAnimation("Walking");
    }
    ...
    public override void FixedUpdate()
    {
        ...
        {   
            ...
            Player.PhysicsBody.AddForce((right + front) * (float)GameTime.FixedDeltaTime * 3000f, 40f * Mathf.Clampy(Mathf.Abs(input.Y) + Mathf.Abs(input.X * 0.6f), 0, 1));
        }   
    }
