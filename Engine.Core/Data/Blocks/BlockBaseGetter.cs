...
public class DefaultVoxelChunkHandler(VoxelChunk chunk) : BaseVoxelChunkHandler(chunk.Position, chunk.Blocks)
{
    ...
    {
        int lx = position.X - Chunk.Position.X;
        int ly = position.Y - Chunk.Position.Y;
        int lz = position.Z - Chunk.Position.Z;

        if ((uint)lx < 32u & (uint)ly < 32u & (uint)lz < 32u)
            return Chunk.Get((lx, ly, lz));

        return Chunk.Renderer.GetChunk(VoxelData.BlockToChunkRelative(position), out var c) ? c.Get((lx, ly, lz)) : Block.Air;
    }
