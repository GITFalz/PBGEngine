...
public unsafe class GFX
{
    ...
    public static Result AllocateDescriptorSets(out DescriptorSet* pDescriptorSets)
    {
        var result = Vk.AllocateDescriptorSets(Device, null, out pDescriptorSets);
        #if DEBUG
        BufferBase.SetDebug(pDescriptorSets);
        #endif
        return result;
    }
    ...
    { 
        ...
        for (int i = 0; i < descriptorSetCount; i++)
            BufferBase.RemoveDebug(pDescriptorSets[i]);
        #endif
    }
    ...
    /// </summary>
    public static void Viewport() => Viewport(CommandBuffer, 0, 0, (uint)VoxelEngine.Width, (uint)VoxelEngine.Height);
