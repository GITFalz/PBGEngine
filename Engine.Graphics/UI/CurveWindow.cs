...
public class CurveWindow
{
    ...
    public CurveWindow(UIController controller, Vector2 position, Vector4 offset, Vector2 size) 
    {
        ...
        X0Button.SetOnClick(() =>
        {
            VoxelEngine.SetCursorState(CursorState.Grabbed);
        ...
        X0Button.SetOnRelease(() => VoxelEngine.SetCursorState(CursorState.Normal));
        ...
        X1Button.SetOnRelease(() => VoxelEngine.SetCursorState(CursorState.Normal));
    ...
    public UIButton AddButton(Vector4 offset)
    {
        ...
        button.SetOnClick(_ =>
        {
            VoxelEngine.Instance.CursorMode = CursorMode.Disabled;
            SelectedButton = button;
            SetDataSelectedButton(button);
        });
        button.SetOnHold(_ => { UpdateButton(button); SetPositionText(button); });
        button.SetOnRelease(_ => VoxelEngine.Instance.CursorMode = CursorMode.Normal);
