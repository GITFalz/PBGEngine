...
namespace PBG.UI
{
    ...
    public class UIController
    {
        ...
        {
            _fbo = new(() => (uint)VoxelEngine.Width, () => (uint)VoxelEngine.Height);

            ShaderInfo planeShaderInfo = new()
            {
                VertexShaderPath = Path.Combine(VoxelEngine.ShaderPath, "vulkan/fullScreen.vert"),
                FragmentShaderPath = Path.Combine(VoxelEngine.ShaderPath, "vulkan/fullScreen.frag")
            };
        ...
        public static void InitControllers(Scene scene)
        {
            ...
            for (int i = 0; i < scene.ActiveUIControllers.Count; i++)
            {
                var controller = scene.ActiveUIControllers[i];
