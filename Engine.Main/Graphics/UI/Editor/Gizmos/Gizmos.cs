...
public abstract class Gizmo
{
    ...
    {
        var info = new ShaderInfo(VoxelEngine.ShaderPath / "gizmo/gizmo.vert", VoxelEngine.ShaderPath / "gizmo/gizmo.frag");
    ...
    public void GenerateWorldSpacePoints()
    {
        ...
        for (int i = 0; i < indices.Length; i += 3)
        {
            ...
            var tvertA = Mathf.WorldToScreen((model * new Vector4(vertA.position, 1f)).Xyz, proj, view, VoxelEngine.Width, VoxelEngine.Height) ?? (0, 0);
            var tvertB = Mathf.WorldToScreen((model * new Vector4(vertB.position, 1f)).Xyz, proj, view, VoxelEngine.Width, VoxelEngine.Height) ?? (0, 0);
            var tvertC = Mathf.WorldToScreen((model * new Vector4(vertC.position, 1f)).Xyz, proj, view, VoxelEngine.Width, VoxelEngine.Height) ?? (0, 0);
