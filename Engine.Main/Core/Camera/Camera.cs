...
namespace PBG.Rendering
{
    ...
    public class Camera : ScriptingNode
    {
        ...
        private Vector3 _oldFront = -Vector3.UnitZ;
        ...
        private bool _updatedViewMatrix = true;
        private bool _updatedProjectionMatrix = true;
        ...
        public void Resize()
        {
            SCREEN_WIDTH = (VoxelEngine.Width - (_left + _right)).Max(1);
            SCREEN_HEIGHT = (VoxelEngine.Height - (_top + _bottom)).Max(1);
            GetProjectionMatrix();
        }

        public Matrix4 GetViewMatrix()
        {
            var view = (Position - _oldPosition) + (front - _oldFront);
        ...
        {
            if (!_updatedViewMatrix && !_updatedProjectionMatrix)
                return;
            ...
            _updatedProjectionMatrix = false;
        }
        ...
        private void RotateCamera()
        {
            ...
            _targetYaw   += mouseDelta.X * 0.01f * HORIZONTAL_SENSITIVITY;
            _targetPitch -= mouseDelta.Y * 0.01f * VERTICAL_SENSITIVITY;
