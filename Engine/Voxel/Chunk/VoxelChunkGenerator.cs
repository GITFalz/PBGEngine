            ...
            for (int x = 0; x < 32; x++)
            {
                for (int y = 0; y < 32; y++)
                {
                    for (int z = 0; z < 32; z++)
                    {
                        ...
                        for (int y1 = -1; y1 <= 1; y1++) 
                        {
                            for (int z1 = -1; z1 <= 1; z1++)
                            {
                                for (int x1 = -1; x1 <= 1; x1++)
                                {
                                    ...
                                    Vector3i worldPos = position + offset + chunk.Position;
                                    if (chunk.InBounds(worldPos))
                                        sideBlocks[ti] = chunk.Get(position + offset);
                                    else
                                        sideBlocks[ti] = chunk.Renderer.GetBlock(worldPos);
                                    ti++;
                                }
                            }
                        }
                        ...
                        {
                            var sideBlock = chunk.Renderer.GetBlock(position + _offsets[side] + chunk.Position);
                            ...
                            {
                                var face = faces[i];
                                vertexData.Add(new(face.GeometryIndex, x | (y << 5) | (z << 10), r, 0));
                            }
                        }
                        ...
                        {
                            var face = iFaces[i];
                            vertexData.Add(new(face.GeometryIndex, x | (y << 5) | (z << 10), 0, 0));
                        }
                    }
                }
            }
