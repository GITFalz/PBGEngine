...
public unsafe static class GreedyMesher7Y
{
    ...
    public static void GetBitMaps(VoxelChunk chunk, ref NeighbourChunks neighbours, int workerId)
    {
        ...
        for (int z = 0; z < 32; z++)
        {
            ...
            for (int x = 0; x < 32; x++)
            {
                ...
                uint bitTop    = neighbours.Blocks22[blockIndex].blockData != 0 ?     1U : 0;
                uint bitBottom = neighbours.Blocks4[blockIndex + 31].blockData != 0 ? 1U : 0;
            ...
            uint* ptr12 = (uint*)neighbours.Blocks12 + indexNx;
            uint* ptr14 = (uint*)neighbours.Blocks14 + indexPx;
    ...
    private static ulong GetSolidRow(uint* ptr, int z, ref V256u sRow1, ref V256u sRow2, ref V256u sRow3, ref V256u sRow4)
    {
        V256u row1 = V256U.Load(ptr);
        V256u row2 = V256U.Load(ptr + 8);
        V256u row3 = V256U.Load(ptr + 16);
    ...
    public static void HandleGreedyFrontAndBack(VoxelChunk chunk, ref MeshMapping mapping, ulong* bitMap, uint* aoTypeMap, uint* typeMap, uint* frontData, uint* backData, int z)
    {
        ...
        Block* blockPtr = chunk.Blocks + z * 1024;
        ...
        for (int i = 0; i < 32; i++)
        {
            // Front
            {
                ...
                while (row != 0)
                {
                    ...
                    Block block = blockPtr[trailingZeros + i * 32];
                    ...
                    while (w < 32 - i)
                    {
                        ...
                        Block sBlock = blockPtr[trailingZeros + iw * 32];
