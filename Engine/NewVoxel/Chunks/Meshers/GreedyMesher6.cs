...
public unsafe static class GreedyMesher6
{
    ...
    public static bool GenerateMesh(VoxelChunk chunk, ref MeshMapping mapping, int workerId)
    {
        ...
        {  
            ...
            HandleGreedyFrontAndBack( blocks, ref mapping, bitMap, aoTypeMap, typeMap, frontSlice, backSlice, i);
            ...
            HandleGreedyTopAndBottom(   blocks, ref mapping, bitMap, aoTypeMap, typeMap, topSlice, bottomSlice, i);
            ...
            HandleGreedyRight(blocks, ref mapping, rightMask, rightType, zTypeMap, rightSlice, i);
            HandleGreedyLeft(  blocks, ref mapping, leftMask, leftType, zTypeMap, leftSlice, i);
    ...
    public static void HandleGreedyFrontAndBack(Block* blocks, ref MeshMapping mapping, ulong* bitMap, ulong* aoTypeMap, uint* typeMap, uint* frontData, uint* backData, int z)
    {
        for (int i = 0; i < 32; i++)
        {
            // Front
    ...
    public static void HandleGreedyRight(Block* blocks, ref MeshMapping mapping, ulong* bitMap, ulong* aoTypeMap, uint* typeMap, uint* data, int x)
    {
        for (int i = 0; i < 32; i++)
        {
    ...
    public static void HandleGreedyLeft(Block* blocks, ref MeshMapping mapping, ulong* bitMap, ulong* aoTypeMap, uint* typeMap, uint* data, int x)
    {
        for (int i = 0; i < 32; i++)
        {
            uint aoType1 = (uint)aoTypeMap[i];
    ...
    public static void HandleGreedyTopAndBottom(Block* blocks, ref MeshMapping mapping, ulong* bitMap, ulong* aoTypeMap, uint* typeMap, uint* topData, uint* bottomData, int y)
    {
        for (int i = 0; i < 32; i++)
        {
            {
