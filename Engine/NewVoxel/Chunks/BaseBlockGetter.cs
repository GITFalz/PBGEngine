...
public unsafe abstract class BaseVoxelChunkHandler(Vector3i worldPosition, Block* blocks)
{
...
public unsafe class DefaultVoxelChunkHandler(VoxelChunk chunk) : BaseVoxelChunkHandler(chunk.WorldPosition, chunk.Blocks)
{
    ...
    public override Block GetBlock(Vector3i position)
    {
        ...
        if ((uint)lx < 32u & (uint)ly < 32u & (uint)lz < 32u)
            return Chunk.Blocks[VoxelChunk.GetIndex(lx, ly, lz)];

        return Chunk.Renderer.GetChunk(VoxelData.BlockToChunkRelative(position), out var c) ? c.Blocks[VoxelChunk.GetIndex(lx, ly, lz)]: Block.Air;
    }
...
public unsafe class DefaultVoxelChunkHandlerNew(VoxelChunk chunk)
{
    public VoxelChunk Chunk = chunk;
    public List<Vector3> Vertices = [];
    public List<Vector3> Normals = [];
