...
public class ColorPicker : ScriptingNode
{
    ...
    */
    ...
    new UICol(left_[Position.X - 5], top_[Position.Y - 30], w_[Picker.Width + 10], h_[Picker.Height + 35])
    .OnHover(_ => Picker.Hovering = true)[
        new UICol(w_full, h_[25], blank_full_g_[30]).OnHold(_ =>
        {
            Vector2 mouseDelta = Input.GetMousePosition();
            if (mouseDelta == Vector2.Zero) return;
            Picker.UpdateColorPickerPosition();
        })[
            new UIImg(w_[20], h_[20], icon_[15], middle_right, right_[2], bg_white, hover_scale_[1.2f], hover_scale_duration_[0.25f], hover_scale_easeout).OnClick(_ =>
            {
        ...
        new UIImg(w_full, h_full_minus_[25], bottom_center, blank_full_g_[20], border_ui_[2, 2, 2, 2], border_color_g_[30]),
        new UICol(w_[Mathf.Min(Picker.Width, Picker.Height - 20)], h_[Mathf.Min(Picker.Width, Picker.Height - 20)], bottom_left, left_[5], bottom_[25])
        .OnHold(_ => SetSliderColor(Input.GetMousePosition().X - Picker.ColorPickerPosition.X, Input.GetMousePosition().Y - (Game.Height - Picker.ColorPickerPosition.Y) + Picker.Height))[
            new UIImg(w_[10], h_[10], right_[5], bottom_[5], depth_[10], blank_full_g_[100], border_ui_[3, 3, 3, 3], border_color_g_[40]).Ref(ref ColorPickSlider)
        ],
        new UIVCol(w_[115], h_[Mathf.Min(Picker.Width, Picker.Height - 20)], top_right, top_[30], right_[5], grow_children, spacing_[5])[
            new UICol(w_[115], h_[30])[
                new UIText("R", middle_left, left_[5]),
                Run(() =>
                {
                    Picker.RedField = new UIField("0", mc_[3], fs_[1], middle_left);
                    return [
                    new UICol(w_full_minus_[60], h_[30], blank_full_g_[10], left_[20], border_[5, 0, 0, 0])[
                        Picker.RedField
                    ],
                    new UICol(w_[20], h_full, top_right, right_[20], blank_full, hover_color_[(0.3f, 0.3f, 0.3f, 0f), (0.3f, 0.3f, 0.3f, 1f)], hover_color_duration_[0.2f], hover_color_easeout)
                    .OnClick(c => SetRGB(Picker.RedField.GetByte() - 1, null, null))[
                        new UIImg(w_[20], h_[20], icon_[2], bg_white, middle_center)
                    ],
                    new UICol(w_[20], h_full, top_right, blank_full, hover_color_[(0.3f, 0.3f, 0.3f, 0f), (0.3f, 0.3f, 0.3f, 1f)], hover_color_duration_[0.2f], hover_color_easeout)
                    .OnClick(c => SetRGB(Picker.RedField.GetByte() + 1, null, null))[
                        new UIImg(w_[20], h_[20], icon_[0], bg_white, middle_center)
                    ]];
                })
            ],
            new UICol(w_[115], h_[30])[
                new UIText("G", middle_left, left_[5]),
                Run(() =>
                {
                    Picker.GreenField = new UIField("0", mc_[3], fs_[1], middle_left);
                    return [
                    new UICol(w_full_minus_[60], h_[30], blank_full_g_[10], left_[20], border_[5, 0, 0, 0])[
                        Picker.GreenField
                    ],
            ...
            new UICol(w_[115], h_[30])[
                new UIText("B", middle_left, left_[5]),
                Run(() =>
                {
                    Picker.BlueField = new UIField("0", mc_[3], fs_[1], middle_left);
                    return [
                    new UICol(w_full_minus_[60], h_[30], blank_full_g_[10], left_[20], border_[5, 0, 0, 0])[
                        Picker.BlueField
                    ],
        ...
        new UICol(w_[Picker.Width], h_[20], bottom_[5], bottom_center)
        .OnHold(_ => SetBarColor(Input.GetMousePosition().X - Picker.ColorPickerPosition.X))[
            new UIImg(w_[10], h_[16], right_[6], bottom_left, top_[4], depth_[10], blank_full, rgb_[1, 0, 0], border_ui_[3, 3, 3, 3], border_color_g_[40]).Ref(ref ColorBarSlider)
        ] 
    ];
