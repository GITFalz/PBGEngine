...
namespace PBG.UI2
{
    public static partial class Styles
    {
        ...
        // VISIBILITY CLASSES
        // =============================================================================
        public readonly static ValueStyle visible = new(e => { e.Visible = true; if (e is IUICol col) col.WasVisible = true; });
        public readonly static ValueStyle invisible = new(e => { e.Visible = false; if (e is IUICol col) col.WasVisible = false; });
        public readonly static ValueStyle hidden = new(e => { e.Visible = false; if (e is IUICol col) col.WasVisible = false; });
        public readonly static ValueStyle not_toggle_old_invisible = new(e => { if (e is IUICol col) col.SetForceToggleVisible(true); });
        ...
        // SPACING CLASSES
        // =============================================================================
        public readonly static UnaryType<float> spacing_ = new((v, e) => { if (e is IUICol col) col.SetSpacing(v); });

        // BORDER CLASSES
        // =============================================================================
        public readonly static QuaternaryStyle<float, float, float, float> border_ = new((left, right, top, bottom, e) => { if (e is IUICol col) col.SetBorder((left, right, top, bottom)); });
        // Individual border sides
        public readonly static UnaryType<float> border_left_ = new((v, e) => { if (e is IUICol col) col.SetBorderX(v); });
        public readonly static UnaryType<float> border_right_ = new((v, e) => { if (e is IUICol col) col.SetBorderZ(v); });
        public readonly static UnaryType<float> border_top_ = new((v, e) => { if (e is IUICol col) col.SetBorderY(v); });
        public readonly static UnaryType<float> border_bottom_ = new((v, e) => { if (e is IUICol col) col.SetBorderW(v); });

        // TYPOGRAPHY CLASSES (fontSize: 1 = 10px, 2 = 20px, so linear scaling)
        // =============================================================================
        public readonly static UnaryType<float> font_size_ = new((v, e) => { if (e is IUIText t) t.FontSize = v; });
        public readonly static UnaryType<float> fs_ = new((v, e) => { if (e is IUIText t) t.FontSize = v; });

        // Text character limits
        public readonly static UnaryType<int> max_chars_ = new((v, e) => { if (e is IUIText t) t.MaxCharCount = v; });
        public readonly static UnaryType<int> mc_ = new((v, e) => { if (e is IUIText t) t.MaxCharCount = v; });
        public readonly static ValueStyle text_align_left = new(e => { if (e is IUIText t) t.TextAlign = TextAlign.Left; });
        public readonly static ValueStyle text_align_center = new(e => { if (e is IUIText t) t.TextAlign = TextAlign.Center; });
        public readonly static ValueStyle text_align_right = new(e => { if (e is IUIText t) t.TextAlign = TextAlign.Right; });

        // LAYOUT BEHAVIOR CLASSES
        // =============================================================================
        public readonly static ValueStyle ignore_invisible = new(e => { if (e is IUICol col) col.SetIgnoreInvisibleElements(true); });
        public readonly static ValueStyle include_invisible = new(e => { if (e is IUICol col) col.SetIgnoreInvisibleElements(false); });
        public readonly static ValueStyle allow_scrolling_to_top = new(e => { if (e is IUICol col) col.SetAllowScrollingToTop(true); });
        public readonly static ValueStyle block_scrolling_to_top = new(e => { if (e is IUICol col) col.SetAllowScrollingToTop(false); });
        public readonly static UnaryType<float> scroll_speed_ = new((v, e) => { if (e is IUICol col) col.SetScrollingSpeed(v); });
        ...
        public readonly static UnaryType<int> texture_ = new((v, e) => { if (e is IUIPanel p) p.TextureID = v; });
        public readonly static UnaryType<int> icon_ = new((v, e) => { if (e is IUIPanel p) p.TextureID = v | 0x20000000; });
        public readonly static UnaryType<string> item_ = new((name, e) =>
        {
            if (ItemDataManager.AllItems.TryGetValue(name, out var item) && e is IUIPanel panel)
                panel.TextureID = item.Index | 0x40000000;  
        });

        public readonly static ValueStyle light_round = new(e => { if (e is IUIPanel p) { p.TextureID = 0; p.Slice = SLICE_100; }});
        public readonly static ValueStyle dark_round = new(e => { if (e is IUIPanel p) { p.TextureID = 1; p.Slice = SLICE_100; }});
        public readonly static ValueStyle blank_round = new(e => { if (e is IUIPanel p) { p.TextureID = 2; p.Slice = SLICE_100; }});

        public readonly static ValueStyle light_sharp = new(e => { if (e is IUIPanel p) { p.TextureID = 10; p.Slice = SLICE_100; }});
        public readonly static ValueStyle dark_sharp = new(e => { if (e is IUIPanel p) { p.TextureID = 11; p.Slice = SLICE_100; }});
        public readonly static ValueStyle blank_sharp = new(e => { if (e is IUIPanel p) { p.TextureID = 12; p.Slice = SLICE_100; }});

        public readonly static ValueStyle light_full = new(e => { if (e is IUIPanel p) { p.TextureID = 20; p.Slice = SLICE_100; }});
        public readonly static ValueStyle dark_full = new(e => { if (e is IUIPanel p) { p.TextureID = 21; p.Slice = SLICE_100; }});
        public readonly static ValueStyle blank_full = new(e => { if (e is IUIPanel p) { p.TextureID = 22; p.Slice = SLICE_100; }});

        /// <summary>
        /// (left, top, right, bottom)
        /// </summary>
        public readonly static QuaternaryStyle<float, float, float, float> border_ui_ = new((x, y, z, w, e) => { if (e is IUIPanel p) { p.BorderUI = (x, y, z, w);} });
        public readonly static QuaternaryStyle<float, float, float, float> border_rgba_ = new((r, g, b, a, e) => { if (e is IUIPanel p) { p.BorderColor = (r, g, b, a);} });
        public readonly static UnaryType<Vector4> border_color_ = new((color, e) => { if (e is IUIPanel p) { p.BorderColor = color; } });
        public readonly static BinaryType<float, int> border_color_g_ = new(
            (value, e) => { if (e is IUIPanel p) { p.BorderColor = new(new(value), 1f); } },
            (value, e) => { if (e is IUIPanel p) { p.BorderColor = new(new(((float)value) / 100f), 1f); } });

        
        public readonly static BinaryType<float, int> light_round_g_ = new(
            (value, e) => { if (e is IUIPanel p) { p.Color = new(new(value), 1f); p.TextureID = 0; p.Slice = SLICE_100; } },
            (value, e) => { if (e is IUIPanel p) { p.Color = new(new(((float)value) / 100f), 1f); p.TextureID = 0; p.Slice = SLICE_100;} });
        public readonly static BinaryType<float, int> dark_round_g_ = new(
            (value, e) => { if (e is IUIPanel p) { p.Color = new(new(value), 1f); p.TextureID = 1; p.Slice = SLICE_100; } },
            (value, e) => { if (e is IUIPanel p) { p.Color = new(new(((float)value) / 100f), 1f); p.TextureID = 1; p.Slice = SLICE_100; } });
        public readonly static BinaryType<float, int> blank_round_g_ = new(
            (value, e) => { if (e is IUIPanel p) { p.Color = new(new(value), 1f); p.TextureID = 2; p.Slice = SLICE_100; } },
            (value, e) => { if (e is IUIPanel p) { p.Color = new(new(((float)value) / 100f), 1f); p.TextureID = 2; p.Slice = SLICE_100; } });


        public readonly static BinaryType<float, int> light_sharp_g_ = new(
            (value, e) => { if (e is IUIPanel p) { p.Color = new(new(value), 1f); p.TextureID = 10; p.Slice = SLICE_100; } },
            (value, e) => { if (e is IUIPanel p) { p.Color = new(new(((float)value) / 100f), 1f); p.TextureID = 10; p.Slice = SLICE_100; } });
        public readonly static BinaryType<float, int> dark_sharp_g_ = new(
            (value, e) => { if (e is IUIPanel p) { p.Color = new(new(value), 1f); p.TextureID = 11; p.Slice = SLICE_100; } },
            (value, e) => { if (e is IUIPanel p) { p.Color = new(new(((float)value) / 100f), 1f); p.TextureID = 11; p.Slice = SLICE_100; } });
        public readonly static BinaryType<float, int> blank_sharp_g_ = new(
            (value, e) => { if (e is IUIPanel p) { p.Color = new(new(value), 1f); p.TextureID = 12; p.Slice = SLICE_100; } },
            (value, e) => { if (e is IUIPanel p) { p.Color = new(new(((float)value) / 100f), 1f); p.TextureID = 12; p.Slice = SLICE_100; } });

        public readonly static BinaryType<float, int> light_full_g_ = new(
            (value, e) => { if (e is IUIPanel p) { p.Color = new(new(value), 1f); p.TextureID = 20; p.Slice = SLICE_100; } },
            (value, e) => { if (e is IUIPanel p) { p.Color = new(new(((float)value) / 100f), 1f); p.TextureID = 20; p.Slice = SLICE_100; } });
        public readonly static BinaryType<float, int> dark_full_g_ = new(
            (value, e) => { if (e is IUIPanel p) { p.Color = new(new(value), 1f); p.TextureID = 21; p.Slice = SLICE_100; } },
            (value, e) => { if (e is IUIPanel p) { p.Color = new(new(((float)value) / 100f), 1f); p.TextureID = 21; p.Slice = SLICE_100; } });
        public readonly static BinaryType<float, int> blank_full_g_ = new(
            (value, e) => { if (e is IUIPanel p) { p.Color = new(new(value), 1f); p.TextureID = 22; p.Slice = SLICE_100; } },
            (value, e) => { if (e is IUIPanel p) { p.Color = new(new(((float)value) / 100f), 1f); p.TextureID = 22; p.Slice = SLICE_100; } });

        public readonly static ValueStyle slice_null = new(e => { if (e is IUIPanel p) { p.Slice = (-1, -1); } });
        public readonly static ValueStyle slice_75 = new(e => { if (e is IUIPanel p) { p.Slice = SLICE_75; } });
        public readonly static ValueStyle slice_100 = new(e => { if (e is IUIPanel p) { p.Slice = SLICE_100; } });
