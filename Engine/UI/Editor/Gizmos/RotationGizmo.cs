...
public class RotationGizmo : Gizmo
{
    ...
    public Vector2 SliderDirection(Axis axis)
    {
        ...
        var tvertA = Mathf.WorldToScreen((new Vector4(start, 1f) * model).Xyz, proj, view, Game.Width, Game.Height) ?? (0, 0);
        var tvertB = Mathf.WorldToScreen((new Vector4(end, 1f) * model).Xyz, proj, view, Game.Width, Game.Height) ?? (0, 0);
    ...
    public bool Update()
    {
        ...
        if (Input.IsMouseDown(MouseButton.Left) && hoveringTriangle != null && Input.MouseDelta != Vector2.Zero)
        {
            ...
            var center = Mathf.WorldToScreen(
                (new Vector4(Position, 1f) * ModelMatrix).Xyz, proj, view, Game.Width, Game.Height
            ) ?? (0, 0);
