...
{
    private UIElementBase FileManager =>
    ...
    public void GenerateCurrentFolderFiles()
    {
                ...
                if (path != null && Path.GetExtension(path).Equals(".json", StringComparison.CurrentCultureIgnoreCase))
                {
                    try
                    {
                        if (SceneSerializer.Deserialize(path))
                        {
                            ...
                            ReloadSceneHierarchy(SceneBlueprint.Blueprint, SceneBlueprint.BlueprintScene);
                        }
                    }
                    catch (Exception ex)
                    {
        ...
        foreach (var directory in directories)
        {
            ...
            elements[i] = new UICol().Class(data_["path", directory], data_["name", fileName], w_[90], h_[90], top_left, blank_sharp, rgba_v4_[Bg2], hover_color_[Bg2, Bg4], hover_color_easeout, hover_color_duration_[0.5f], hover_scale_[1.1f], hover_scale_easeout, hover_scale_duration_[0.5f])
            .OnClick(col =>
            {
    ...
    private void OnFileReleased(UICol c)
    {
        ...
        if (HoveringOverInspector && SceneBlueprint.SelectedNode != null && ext == ".cs")
        {
            SceneBlueprint.SelectedNode.AddScript(c.Dataset.String("name"));
            RefreshInspectorPanel(SceneBlueprint.SelectedNode, SceneBlueprint.SelectedNode.Transform);
        }
        ...
        if (HoveringCenter && ext == ".obj")
        {
            ...
            SceneDefinitionNode modelNode;
            if (SceneBlueprint.SelectedNode != null)
            {
                modelNode = SceneBlueprint.SelectedNode;
            }
            else
            {
                modelNode = SceneDefinition.Blueprint.AddOrGetNode("Models");
            }
